...
async function fetch_collection(base_url: string, kind: DynamicAssetKind, reeman_url?: string): Promise<Candidate[]> {
	const server_url = reeman_url || base_url;
	const records: CandidateRecord[] = [];
	let offset = 0;
	let total = 0;

	do {
		const collection_url = new URL(`/${kind}`, server_url);
...
async function download_candidate(
	...
	const source_url = new URL(`/${kind}/${encoded_path}`, server_url);
...
export async function sync_dynamic_assets(options: DynamicAssetSyncOptions): Promise<DynamicAssetSyncResult> {
	const [image_candidates, file_candidates] = await Promise.all([
		fetch_collection(options.base_url, "images", options.reeman_url),
		fetch_collection(options.base_url, "files", options.reeman_url),
	]);
