 ...
 *   dev/resolve.ts      - request URL → locale + template/markdown file
...
import { active_locales, default_locale, locales } from "$config/supported_locales";
...
import { stamp_dev_source } from "$root/src/lib/inspector_stamp";
...
import { canonical_redirect_target, resolve_request, resolve_template } from "./dev/resolve";
...
declare global {
	var __dev_console_tagged: boolean | undefined;
}

// scripts/dev.ts runs with stdio: "inherit" under orchestrate.ts (not piped
// through its prefixed_pipe like [tw]/[sync]/[img]/[css]), so the "press 'o'
// to open in browser" feature (server_controls.ts) can read the real
// terminal stdin. That means every console.log/error/warn call anywhere in
// this process's module graph needs a [dev] tag applied here, once, instead
// of at each call site - guarded by a globalThis flag so `bun --hot`
// re-evaluating this module doesn't wrap the same console methods twice.
if (!globalThis.__dev_console_tagged) {
	globalThis.__dev_console_tagged = true;
	const DEV_TAG = "\x1b[36m[dev]\x1b[0m";
	...
	console.log = (...args: unknown[]) => original_log(DEV_TAG, ...args);
	console.error = (...args: unknown[]) => original_error(DEV_TAG, ...args);
	console.warn = (...args: unknown[]) => original_warn(DEV_TAG, ...args);
}
...
if (!is_first_run) {
	...
	const engine = new TemplateEngine({
		...
		// views_dir is src/public, so the engine's default (its parent, src/)
		// would stamp "public/index.ree" while /__ree_open and the inspector's
		// save resolve against the repo root.
		project_root,
	});

	// Index components at startup so a duplicate tag name is reported once, here,
	// instead of on whichever request first renders that tag. Cache is off, so
	// nothing is compiled and hot reload is unaffected; components added later
	// still resolve through the engine's dev self-heal.
	...
	const server = Bun.serve(
		{
			...
			fetch: async (req: Request): Promise<Response | undefined> => {
				...
				// canonical full-locale URL before any route resolution happens.
				const request_url = new URL(req.url);
				const redirect_target = canonical_redirect_target(pathname, request_url.search);
				if (redirect_target) return Response.redirect(redirect_target, 301);

				const { locale, path: canonical } = resolve_request(pathname);

				// Pagination routes (page 1 and /page/N) take precedence.
				const pmatch = match_pagination(canonical, locale, state);
				if (pmatch) return await render_pagination(ctx, pmatch, locale);

				// Template / markdown resolution.
				const resolved = resolve_template(canonical, locale, state);
				if (resolved?.kind === "ree") {
					...
					return await render_ree(ctx, resolved.rel_path, locale);
				}
	...
	console.log(`    🌐 Locales: ${active_locales.map((l) => `${l}${l === default_locale ? " (default)" : ""}`).join(
		", "
	)}`);
