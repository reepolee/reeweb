...
describe("index_prefix_for", () => {
	test("uses the root index for the default locale", () => {
		expect(index_prefix_for("sl-si", { prefix: "", brand: "" })).toBe("");
	});

	test("writes non-default locale indexes below their locale prefix", () => {
		expect(index_prefix_for("en-us", { prefix: "/docs", brand: "Docs" })).toBe("/en-us/docs");
	});
});
