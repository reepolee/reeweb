...
function default_template_data(user_data: Record<string, any>): Record<string, any> {
	const locale = typeof user_data.locale === "string" ? user_data.locale : default_locale;
	const locale_urls = Object.fromEntries(locales.map((l) => [
		l,
		l === default_locale ? "" : `/${locale_url_segment(l)}`,
	]));

	return build_page_data({
		locale,
		locale_url_prefix: locale_urls[locale] ?? "",
		request_url: "/",
		canonical_path: "/",
		hreflang_links: [],
		site_name: pkg.name,
...
export const template_tools: ToolDef[] = [
	{
		...
		inputSchema: {
			...
			properties: {
				...
				data: {
					type: "object",
					description: "Data object to pass to the template (common vars like locale, is_dev are auto-injected)",
	...
	{
		name: "render_page",
		description: "Render one site route byte-identical to a full SSG build (real translations, layouts, data loaders, pagination). Pass a request path like '/', '/blog' or '/en-us/about'. Requires MCP_ENABLE_TEMPLATE_RENDER=true.",
		inputSchema: {
			type: "object",
			properties: {
				url: {
					type: "string",
					description: "Request path starting with '/' (locale prefix optional, lowercased e.g. 'en-us')",
				},
			},
			required: ["url"],
		},
