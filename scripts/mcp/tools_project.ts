...
export const project_tools: ToolDef[] = [
	...
	{
		name: "list_pages",
		description: "List all site pages (.ree and .md under src/public) with canonical route, kind, translation namespace, draft flag, locale-variant files, and data-loader presence",
		inputSchema: { type: "object", properties: {} },
		handler: async () => {
	...
	},
	{
		...
		description: "Get detailed info about one page by canonical route (e.g. '/about') - files, frontmatter, localized URLs per locale, pagination flag",
		inputSchema: {
			type: "object",
			properties: {
				route: {
	...
	{
		name: "get_route_map",
		description: "Map every canonical route to its localized output URL per locale (route_name substitution + locale prefix), as built by the SSG",
		inputSchema: { type: "object", properties: {} },
		handler: async () => {
	...
	},
	{
		...
		description: "List configured locales and their top-level translation namespaces (from the per-locale JSON files under src/public)",
		inputSchema: { type: "object", properties: {} },
		handler: async () => {
			const locales = await list_translation_namespaces();
			return json_content({ locales });
		},
	},
	{
		name: "get_translations",
		description: "Get the merged translation tree for a locale, optionally narrowed to a dot-separated namespace (e.g. 'routes', 'blog', 'routes.ui')",
		inputSchema: {
			type: "object",
			properties: {
				locale: { type: "string", description: "Lowercase BCP 47 locale code (e.g. 'en-us', 'sl-si')" },
				namespace: {
					type: "string",
					description: "Optional dot-separated namespace path. If omitted, returns the full tree for the locale.",
				},
			},
			required: ["locale"],
		},
		handler: async (args) => {
			const translations = await get_translations_for(args.locale, args.namespace);
			return json_content(translations);
		},
	},
	{
		name: "get_config",
		description: "Show project configuration - locales (all/active/default/soft-launch), pagination, redirects, page/component counts",
		inputSchema: { type: "object", properties: {} },
		handler: async () => {
