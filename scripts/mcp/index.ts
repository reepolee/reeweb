 ...
 * - get_route_map         Canonical route → localized output URL per locale
 * - list_translations     Locales and translation namespaces (JSON files)
 * - get_translations      Translation tree for a locale / namespace
 * - get_config            Locales, pagination, redirects, counts
 ...
 * - check_translations    Cross-locale gaps, missing keys, orphans (read-only)
 * - set_translations      Upsert entries into {locale}.json files
 * - add_locale            Register + seed a new locale
 * - remove_locale         Unregister + delete a locale's JSON files
 *
 * Rendering tools require MCP_ENABLE_TEMPLATE_RENDER=true (they execute local
 * code); run_ssg, set_translations, add_locale, and remove_locale require
