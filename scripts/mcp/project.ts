 ...
 * route map, translations (per-locale JSON files), components, config, code
...
export type PageInfo = {
	/** Canonical page file relative to src/public (locale variants collapsed). */
	path: string;
	...
	draft: boolean;
	/** Actual files on disk backing this page (base + per-locale variants). */
	files: string[];
...
export function list_pages(): PageInfo[] {
	...
	const pages = collect_page_files(PUBLIC_DIR, locales);
	...
	const infos = pages.map((rel): PageInfo => {
		...
		const variant_re = new RegExp(`^${escape_regex(base)}(\\.(${locale_group}))?\\.(ree|md)$`, "i");
...
// ---------------------------------------------------------------------------
// Route map (canonical → localized output URL per locale)
// ---------------------------------------------------------------------------
...
export async function get_route_map(): Promise<Record<string, Record<string, string>>> {
	...
	for (const [canonical, per_locale] of route_map) {
		...
		for (const [locale, localized] of per_locale) {
			const prefix = locale === default_locale ? "" : `/${locale_url_segment(locale)}`;
			urls[locale] = localized === "/" ? (prefix || "/") : `${prefix}${localized}`;
		}
		out[canonical] = urls;
	}
	return out;
...
export async function get_page_detail(route: string): Promise<Record<string, any>> {
	...
	// The collapsed base path may not exist on disk when only locale variants
...
// ---------------------------------------------------------------------------
// Translations (per-locale JSON files under src/public)
// ---------------------------------------------------------------------------
...
export async function list_translation_namespaces(): Promise<Record<string, string[]>> {
	...
	for (const locale of locales) {
		out[locale] = Object.keys(translations[locale] ?? {}).sort();
	}
	return out;
}

export async function get_translations_for(locale: string, namespace?: string): Promise<Record<string, any>> {
	if (!(locales as readonly string[]).includes(locale)) {
		throw new Error(`Unknown locale "${locale}". Configured locales: ${locales.join(", ")}`);
	}
...
export async function get_project_config(): Promise<Record<string, any>> {
	...
	return {
		...
		locales: {
			...
			soft_launch: soft_launch_locales,
