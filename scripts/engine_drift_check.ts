 ...
 * sibling checkout, e.g. ../reepolee-dev). Each project forks the shared
 * engine as its needs grow, so logic divergence is expected and fine. This
 * script reports where the local copy has drifted so the difference stays
 * visible - it does not fail the build unless --enforce is passed.
 *
 ...
 *
 * Usage: bun run engine:check [--verbose|-v] [--enforce]
 */
...
const SHARED_ENGINE_FILES = [
	...
	"template/types.ts",
	"template/precompile.ts",
	"template/translation_path.ts",
];

const VERBOSE = Bun.argv.includes("--verbose") || Bun.argv.includes("-v");
// Informational by default: drift is reported and written to engine_drift.diff
// but does not fail. Pass --enforce (e.g. in CI) to exit non-zero on drift.
...
async function main() {
	...
	if (drifted.length > 0) {
		warn(`${drifted.length} engine file(s) differ in fingerprint from canonical.`);
		info(
			"Divergence is expected: each project forks the shared engine as its needs grow."
		);
		info(
			"Review engine_drift.diff to judge whether any change is unwanted."
		);
		info(
	...
	process.exit(ENFORCE && (drifted.length > 0 || missing.length > 0) ? 1 : 0);
}
