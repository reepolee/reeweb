 ...
 * expand_includes_tracked) so previews and builds inline the same file contents.
 *
...
import type { process_docs_markdown } from "$lib/markdown_docs";
 ...
 *
 * Also returns `host_line_for`: for each 0-based line of the expanded output, the
 * 1-based line in the original body it came from. Lines synthesized from an
 * @include (the wrapping fences + the file body) all map to the @include
 * directive's own host line. Used by the dev inspector to stamp .md blocks with
 * authored source lines despite expansion drift.
 */
