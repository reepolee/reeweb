/**
 * scripts/shared/translation_merge.ts
 *
 ...
 * before rendering a template. Shared by the SSG (scripts/ssg/) and the dev
 * server (scripts/dev/) - moved out of ssg/ so dev does not depend on the
 * SSG phase.
 */
 ...
 * `routes` bundle for `locale`, overlaid with the route's own namespace.
 */
export function merge_route_strings(translations: Record<string, any>, locale: string, namespace: string): Record<string, any> {
	const global_strings = translations[locale]?.routes ?? {};
	const route_strings = namespace ? get_nested(translations[locale], namespace) : {};
