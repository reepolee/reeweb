...
// Demo-only keys in the root en-us.json/sl-si.json. ui.pagination.* and the
// locale-switcher names are shared infrastructure and are kept.
...
function strip_demo_translation_keys(public_dir: string, locale: string): { file: string; keys_removed: string[]; } | null {
	const file_path = join(public_dir, `${locale}.json`);
	...
	return { file: `src/public/${locale}.json`, keys_removed };
}
...
export function remove_demo_content(public_dir: string = PUBLIC_DIR): DemoContentReport {
	...
	for (const locale of locales) {
		const result = strip_demo_translation_keys(public_dir, locale);
