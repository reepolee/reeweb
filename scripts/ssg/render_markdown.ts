 ...
 * the template engine - once per locale with the localized fallback chain.
 *
 * SEO nuance: pages with `localize: false` have byte-identical content in every
 * locale, so non-default variants canonicalize to the default URL and are
...
/**
 * Render a single markdown file for one locale and write it to dist/.
 ...
 */
export async function render_md_file_for_locale(ctx: BuildContext, base_rel_path: string, locale: string, md_files: string[], sidebar_map: Map<string, Map<string, SidebarEntry[]>>): Promise<{ output_rel: string; verbose_label: string; request_url: string; } | null> {
	const { engine, options, locales, default_locale, route_resolver } = ctx;
	const { localized_url_for_locale, resolve_localized_path } = route_resolver;
	const public_dir = options.public_dir;

	const canonical_path = template_to_canonical(base_rel_path);

	const resolved = await resolve_md_file(base_rel_path, locale, default_locale, public_dir);
	...
	const localized_path = resolve_localized_path(canonical_path, locale);
	const { output_rel, verbose_label, locale_url_prefix, request_url, is_default } = output_target(
		localized_path,
		locale,
		default_locale
	);
	...
	const localized = page_is_localized(frontmatter) && (project_hooks.is_localized_path?.(canonical_path, locale) ?? true);
	...
	const hreflang_links = localized ? build_hreflang_links({
		...
		default_locale,
		url_for_locale: (l) => localized_url_for_locale(canonical_path, l),
	}) : [];

	// A page is its own canonical, except a non-default variant of a
	// non-localized page, which canonicalizes to the default-locale URL.
	const canonical_self = is_default || localized;
	const canonical_url = options.site_url ? abs_url(options.site_url, canonical_self ? request_url : localized_url_for_locale(
		canonical_path,
		default_locale
	)) : "";
	...
	const data = build_page_data(
		ctx,
		{
			...
			canonical_url,
			hreflang_links,
			site_name: String(frontmatter.site_name || ctx.translations[locale]?.routes?.site_name || ""),
		},
		{
			...ctx.translations[locale]?.routes,
			sidebar: sidebar_links_for(canonical_path, locale, sidebar_map, ctx),
			body: shaped.body,
			...shaped.fields,
			...frontmatter,
			...
			translations: ctx.translations[locale]?.routes,
		}
	);
...
/**
 * Render every markdown file across all locales. `sidebar_map` supplies
...
export async function render_markdown_files(ctx: BuildContext, md_files: string[], sidebar_map: Map<string, Map<string, SidebarEntry[]>>): Promise<RenderTally> {
	...
	for (const base_rel_path of md_files) {
		...
		for (const locale of locales) {
			try {
				const result = await render_md_file_for_locale(ctx, base_rel_path, locale, md_files, sidebar_map);
				if (!result) continue;
				rendered++;
				...
				console.error(`    ✗ ${locale}/${base_rel_path}: ${msg}`);
			}
		}
	}
