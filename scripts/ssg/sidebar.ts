...
export function sidebar_links_for(canonical_path: string, locale: string, sidebar_map: SidebarMap, ctx: BuildContext): SidebarLink[] | undefined {
	return sidebar_links_for_core(canonical_path, locale, sidebar_map, (cp, l) => ctx.route_resolver.localized_url_for_locale(
		cp,
		l
	));
}
