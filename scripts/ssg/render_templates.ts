 ...
 * per locale. Paginated route indexes are skipped here (the pagination phase
...
/**
 * Render a single .ree template for one locale and write it to dist/.
 * Shared by the batch phase (`render_ree_templates`) and the single-page
 * print-url path, so both stay byte-for-byte identical.
 */
export async function render_ree_template_for_locale(ctx: BuildContext, rel_path: string, locale: string, template_data_map: Map<string, Map<string, Record<string, any>>>): Promise<{ output_rel: string; verbose_label: string; request_url: string; }> {
	...
	const localized = project_hooks.is_localized_path?.(canonical_path, locale) ?? true;
	...
	const data = build_page_data(
		...
		},
		{
			...
			...template_data_map.get(rel_path)?.get(locale),
			translations: merged,
		}
	);
...
/**
 * Render every .ree template (except paginated indexes) across all locales.
 * `template_data_map` supplies per-template dynamic data loaded from sibling
 * .ts files, keyed template -> locale. Generated request URLs are recorded
 * on `ctx.generated_routes`.
 */
export async function render_ree_templates(ctx: BuildContext, ree_files: string[], paginated_index_rels: Set<string>, template_data_map: Map<string, Map<string, Record<string, any>>>): Promise<RenderTally> {
	const { locales } = ctx;
	...
	for (const rel_path of ree_files) {
		...
		for (const locale of locales) {
			try {
				const { verbose_label } = await render_ree_template_for_locale(ctx, rel_path, locale, template_data_map);
				rendered++;
