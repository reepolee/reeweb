 ...
 *
 * Loaded once per (template, locale): a loader that fetches from reepolee
 * gets the locale it is rendering for, and reepolee serves that locale's rows
 * from its locale-suffixed tables. Loading once per template and reusing the
 * result across locales would ship the default locale's content everywhere.
 */
async function load_template_data_map(ree_files: string[], paginated_index_rels: Set<string>, public_dir: string, locales: readonly string[], verbose: boolean): Promise<Map<string, Map<string, Record<string, any>>>> {
	const template_data_map = new Map<string, Map<string, Record<string, any>>>();
	...
	for (const rel_path of ree_files) {
		...
		try {
			...
			if (typeof data_module.load_template_data === "function") {
				// Each locale is an independent fetch - run them concurrently.
				const load_for_locale = async (locale: string): Promise<[string, Record<string, any>]> => {
					const result = await data_module.load_template_data({ locale });
					return [locale, result ?? {}];
				};
				const entries = await Promise.all(locales.map(load_for_locale));
				template_data_map.set(rel_path, new Map(entries));
				console.log(`    ✓ Data loaded for ${rel_path} (${locales.length} locale(s))`);
			} else {
...
export async function run_ssg(options: BuildOptions = parse_args()): Promise<{ rendered: number; errors: number; }> {
	...
	console.log(`    ✓ ${locales.length} locale(s) loaded`);

	// Self-names for the switcher come from each locale's own translation file
	// (en-us.json says "English", sl-si.json says "Slovenščina").
	const locale_self_names: Record<string, string> = {};
	for (const locale of locales) {
		locale_self_names[locale] = translations[locale]?.routes?.ui?.language_names?.[locale] ?? locale;
	}

	// URL prefixes: default locale at root (""), others at "/{locale}" (lowercased).
	const locale_urls: Record<string, string> = {};
	for (const locale of locales) {
		locale_urls[locale] = locale === default_locale ? "" : `/${locale_url_segment(locale)}`;
	}
	...
	// Index every component up front. The compile pass is gated on engine.cache
	// (off here), so this only builds the ReeTag/locale-variant indexes - but it
	// scans the whole tree, so two components sharing a tag name fail the build
	// now and are all reported together. Without it the clash only surfaces if
	// and when some page happens to use that tag.
	...
	const all_page_files = without_draft_pages(collect_page_files(public_dir, locales));
	...
	//
	// Exception: an `_`-prefixed `.ts` is build-time source by convention and has
	// no sibling template - `blog/_schema.ts` (content-collection schema) is the
	// shipped example. Publishing it would put project source on the web.
	...
	for (const file of all_files) {
		...
		const base_name = file.replace(/^.*[/\\]/, "");
		const is_private_ts = file.endsWith(".ts") && base_name.startsWith("_");
		const is_source_ts = file.endsWith(".ts") && (ree_path_set.has(data_sibling_ree) || is_private_ts);
	...
	const route_map = build_static_route_map(translations, [...ree_files, ...md_files], locales);
	...
	const ctx: BuildContext = {
		...
		locale_urls,
		translations,
		route_resolver: create_route_resolver(route_map, default_locale),
	...
	// locale; correct it by the actual page count produced.
	const flat_total = (ree_files.length + md_files.length) * locales.length;
