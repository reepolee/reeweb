...
describe("merge_route_strings", () => {
	// Route namespaces resolve on translations[locale] directly (sibling to `routes`),
	// while the global strings come from translations[locale].routes.
	const translations = { "sl-si": { routes: { ui: { nav: "Meni" } }, blog: { title: "Blog SL" } } };

	test("overlays the route namespace on top of global routes", () => {
		const merged = merge_route_strings(translations, "sl-si", "blog");
	...
	test("empty namespace yields just the global routes (cloned, not shared)", () => {
		const merged = merge_route_strings(translations, "sl-si", "");
		expect(merged).toEqual({ ui: { nav: "Meni" } });
		merged.ui.nav = "mutated";
		expect(translations["sl-si"].routes.ui.nav).toBe("Meni");
	});
});
