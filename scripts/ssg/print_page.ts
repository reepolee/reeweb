 ...
 * request URL to a locale + template/markdown/pagination match the way the
 * dev server does, then renders only that page through the same render
 * functions the batch phases use (render_ree_template_for_locale /
 * render_md_file_for_locale / render_pagination_page), so the output is
...
import { render_md_file_for_locale } from "./render_markdown";
import { render_pagination_page } from "./render_pagination";
import { render_ree_template_for_locale } from "./render_templates";
...
/** Parse a request path into { locale, path } the way the dev server does. */
function split_locale_prefix(request_path: string): { locale: string; canonical_guess: string; } {
	...
	if (matched_locale) {
		...
		return { locale: matched_locale, canonical_guess: rest.length > 0 ? "/" + rest.join("/") : "/" };
	}

	return { locale: default_locale, canonical_guess: normalized };
}

/** Match a locale-stripped path to a pagination route + page number. */
function match_pagination(localized_path: string, locale: string, canonical_to_localized: (canonical: string, locale: string) => string): { route_dir: string; page: number; } | null {
	...
	for (const route of pagination_config.routes) {
		...
		const localized_base = canonical_to_localized(canonical_base, locale);
...
export async function print_single_page(options: BuildOptions, request_url: string): Promise<string> {
	...
	const engine = new TemplateEngine({ views: public_dir, ext: ".ree", cache: false, auto_escape: true, locales, default_locale, helper_names: DEFAULT_HELPER_NAMES });
	...
	const { locale, canonical_guess } = split_locale_prefix(request_url);

	// Pagination routes take precedence, mirroring the dev server's dispatch order.
	const pagination_match = match_pagination(
		canonical_guess,
		locale,
		(canonical, l) => route_resolver.localized_url_for_locale(canonical, l)
	);
	if (pagination_match) {
		...
		const result = await render_pagination_page(ctx, route, locale, pagination_match.page, all_page_files);
	...
	if (!canonical_to_template.has(canonical_path)) {
		const resolved = route_resolver.resolve_canonical_from_localized(canonical_guess, locale);
	...
	if (template.endsWith(".ree")) {
		...
		if (existsSync(data_full_path)) {
			...
			if (typeof data_module.load_template_data === "function") {
				// Only the one locale being printed is loaded.
				const result = (await data_module.load_template_data({ locale })) ?? {};
				template_data_map.set(template, new Map([[locale, result]]));
			}
		}
		const result = await render_ree_template_for_locale(ctx, template, locale, template_data_map);
		return result.output_rel;
	}

	const sidebar_map = await build_sidebar_map(md_files, ctx);
	const result = await render_md_file_for_locale(ctx, template, locale, md_files, sidebar_map);
