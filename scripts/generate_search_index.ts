 ...
 * `noindex: true` pages are rendered but excluded. Every configured locale
 * gets its own index, so search results stay in the visitor's current locale.
 */
...
// Re-export the extraction helpers so tests and callers keep one import path.
export {
	apply_strip,
	extract_article,
	extract_main,
...
export type { SearchRecord } from "./search_text";
import { html_to_text, page_records, type SearchRecord } from "./search_text";
...
/** URL prefix where one source's index is written for a locale. */
export function index_prefix_for(locale: string, source: SearchSource): string {
	const locale_prefix = locale === default_locale ? "" : `/${locale_url_segment(locale)}`;
	return locale_prefix + source.prefix;
}
...
async function build_source_index(
	...
	const index_prefix = index_prefix_for(locale, source);
	const output_path = join(dist_dir, index_prefix, "search-index.json");
	await Bun.write(output_path, JSON.stringify({ site: index_prefix, records }));
...
async function main() {
	...
	const page_files = without_draft_pages(collect_page_files(public_dir, locales));
	const route_map = build_static_route_map(translations, page_files, locales);

	// Per-locale URL for a canonical path, mirroring the sitemap's resolution
	// so localized routes land on the URL that was actually rendered.
	function url_for(locale: string, canonical: string): string {
		const localized_path = route_map.get(canonical)?.get(locale) ?? canonical;
		const locale_prefix = locale === default_locale ? "" : `/${locale_url_segment(locale)}`;
		return locale_prefix + localized_path;
	}
	...
	for (const locale of locales) {
		for (const source of search.sources) {
			const result = await build_source_index(
				source,
				locale,
				page_files,
				public_dir,
				dist_dir,
				(canonical) => url_for(locale, canonical),
				now,
			);
			total_records += result.records;
			const label = index_prefix_for(locale, source) || "/";
	...
	console.log("=".repeat(50));
	console.log(`Search indexes written (${locales.length} locale(s), ${search.sources.length} source(s), ${total_records} records)`);
}
