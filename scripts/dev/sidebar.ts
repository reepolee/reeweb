...
export function sidebar_links_for(canonical_path: string, locale: string, sidebar_map: SidebarMap, state: SiteState): SidebarLink[] | undefined {
	return sidebar_links_for_core(canonical_path, locale, sidebar_map, (cp, l) => state.localized_url_for_locale(
		cp,
		l
	));
}
