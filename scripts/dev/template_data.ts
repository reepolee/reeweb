...
export async function load_template_data(rel_path: string, public_dir: string, locale: string): Promise<Record<string, any>> {
	...
	try {
		...
		if (typeof data_module.load_template_data === "function") {
			// `locale` lets the loader fetch locale-correct content from reepolee.
			return (await data_module.load_template_data({ locale })) ?? {};
		}
	} catch (err) {
