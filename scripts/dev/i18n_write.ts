 ...
 * was being viewed, the active locale, a dotted key (e.g. "ui.welcome_title")
 * and a new value, resolve the physical {locale}.json file that owns the key
 * and write it back, creating the key if it only existed via cross-locale
 * fallback.
 *
 * File resolution mirrors how load_all_translations assembles namespaces:
 *   - routes (global) strings live in the translations-root {locale}.json
 *     (public_dir/{locale}.json).
 *   - a page under a folder also has that folder's {locale}.json overlaid.
 ...
 * else the routes file - i.e. "edit on the /en-us/ page writes English".
 */

import { existsSync, readFileSync } from "node:fs";
import { dirname, join } from "node:path";

/** Candidate {locale}.json files for a page, most specific first. */
export function candidate_files(public_dir: string, page_rel_path: string, locale: string): string[] {
	const files: string[] = [];
	const page_dir = dirname(page_rel_path);
	if (page_dir && page_dir !== "." && page_dir !== "") {
		files.push(join(public_dir, page_dir, `${locale}.json`));
	}
	files.push(join(public_dir, `${locale}.json`));
	return files;
}
...
export function resolve_i18n_target(public_dir: string, page_rel_path: string, locale: string, key: string): I18nResolveResult {
	if (!/^[A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*)*$/.test(key)) {
		return { ok: false, reason: `invalid key: ${key}` };
	}
	const candidates = candidate_files(public_dir, page_rel_path, locale);
...
export async function write_i18n_value(
	...
	const resolved = resolve_i18n_target(public_dir, page_rel_path, locale, key);
