 ...
 * is viewing plus a dotted key; the server resolves the locale + template from
...
/** Resolve a viewed page URL to its locale + template rel_path, or null. */
function page_for_url(url: string, state: SiteState): { locale: string; rel_path: string; } | null {
	const pathname = new URL(
		url,
		"http://localhost",
	).pathname;
	const request = resolve_request(pathname);
	const resolved = resolve_template(request.path, request.locale, state);
	if (!resolved) return null;
	return { locale: request.locale, rel_path: resolved.rel_path };
}
...
export async function handle_i18n_message(ws: ServerWebSocket, raw: string, state: SiteState): Promise<boolean> {
	...
	if (msg.type === "i18n_get") {
		const target = resolve_i18n_target(state.public_dir, page.rel_path, page.locale, msg.key);
		if (!target.ok) {
			ws.send(JSON.stringify({
				type: "i18n_value",
				id: msg.id,
