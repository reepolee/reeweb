...
/**
 * Match a locale-stripped, slash-normalized path to a pagination route+page.
 *   /blog          → page 1
 *   /blog/page/2   → page 2   (a non-numeric tail won't match - that's a slug)
 */
export function match_pagination(path: string, locale: string, state: SiteState): PaginationMatch | null {
	...
	for (const route of pagination_config.routes) {
		...
		const localized_base = state.resolve_localized_path(canonical_base, locale);
...
export async function render_pagination(ctx: DevContext, match: PaginationMatch, locale: string): Promise<Response> {
	...
	const page_url = make_paginated_url(
		state.localized_url_for_locale(canonical_base, locale),
		pagination_config.path_segment
	);

	const merged = state.merge_strings(locale, path_to_namespace(index_rel));
	const global_strings = state.translations[locale]?.routes ?? {};
