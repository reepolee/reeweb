...
beforeAll(async () => {
	...
	await Bun.write(join(pub, "en-us.json"), JSON.stringify({
		ui: { welcome: "Welcome" },
		nav: { home: "Home" },
	}, null, "\t") + "\n");
	// namespaced (about) file
	await Bun.write(
		join(pub, "about", "en-us.json"),
...
describe("candidate_files", () => {
	test("namespace file first, then routes file", () => {
		const files = candidate_files(pub, "about/index.ree", "en-us");
		expect(files[0]).toBe(join(pub, "about", "en-us.json"));
		expect(files[1]).toBe(join(pub, "en-us.json"));
	});

	test("root page only has the routes file", () => {
		const files = candidate_files(pub, "index.ree", "en-us");
		expect(files).toEqual([join(pub, "en-us.json")]);
	});
});

describe("resolve_i18n_target", () => {
	test("resolves a routes key from the root file", () => {
		const r = resolve_i18n_target(pub, "index.ree", "en-us", "ui.welcome");
		expect(r.ok).toBe(true);
		if (r.ok) {
			expect(r.file).toBe(join(pub, "en-us.json"));
			expect(r.current).toBe("Welcome");
		}
	});

	test("prefers the namespace file when it defines the key", () => {
		const r = resolve_i18n_target(pub, "about/index.ree", "en-us", "heading");
		expect(r.ok).toBe(true);
		if (r.ok) expect(r.file).toBe(join(pub, "about", "en-us.json"));
	});

	test("a missing key targets the most specific existing file", () => {
		const r = resolve_i18n_target(pub, "about/index.ree", "en-us", "brand_new");
		expect(r.ok).toBe(true);
		if (r.ok) {
			expect(r.file).toBe(join(pub, "about", "en-us.json"));
			expect(r.current).toBeUndefined();
		}
	});

	test("rejects an invalid key", () => {
		const r = resolve_i18n_target(pub, "index.ree", "en-us", "ui.a b");
...
describe("write_i18n_value", () => {
	test("updates an existing key, preserving siblings", async () => {
		const w = await write_i18n_value(pub, "index.ree", "en-us", "ui.welcome", "Hi there");
		expect(w.ok).toBe(true);
		const json = await Bun.file(join(pub, "en-us.json")).json();
		expect(json.ui.welcome).toBe("Hi there");
		expect(json.nav.home).toBe("Home"); // untouched
	});

	test("creates a missing key", async () => {
		const w = await write_i18n_value(pub, "index.ree", "en-us", "ui.tagline", "Build fast");
		expect(w.ok).toBe(true);
		const json = await Bun.file(join(pub, "en-us.json")).json();
	...
	test("writes a namespaced key into the namespace file", async () => {
		const w = await write_i18n_value(pub, "about/index.ree", "en-us", "heading", "About Reeweb");
		expect(w.ok).toBe(true);
		const ns = await Bun.file(join(pub, "about", "en-us.json")).json();
		expect(ns.heading).toBe("About Reeweb");
		// The routes file must not gain the key.
		const root = await Bun.file(join(pub, "en-us.json")).json();
		expect(root.heading).toBeUndefined();
	});

	test("preserves tab indentation instead of reformatting to spaces", async () => {
		await write_i18n_value(pub, "index.ree", "en-us", "ui.welcome", "Hi again");
		const text = await Bun.file(join(pub, "en-us.json")).text();
	...
	test("preserves space indentation when the source file uses spaces", async () => {
		const space_file = join(pub, "space-locale.json");
		await Bun.write(space_file, JSON.stringify({ ui: { hello: "Hello" } }, null, 4) + "\n");
		await write_i18n_value(pub, "index.ree", "space-locale", "ui.hello", "Hi there");
