...
import { stage_and_mirror_release_files, update_override_hashes } from "./release_files";
...
async function release(): Promise<void> {
	...
	if (args.includes("--update-hashes")) {
		const updated = update_override_hashes(PROJECT_ROOT);
		if (updated.length === 0) {
			console.log("All override hashes are up to date.");
		} else {
			for (const rel of updated) console.log(`   ${rel}  ->  updated`);
			console.log(`${updated.length} override hash(es) updated.`);
		}
		return;
	}
