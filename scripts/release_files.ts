...
/** Rewrites the `@release-sync-hash` comment in every `.override.*` file to match its current original. Returns the updated paths (relative to `source_dir`). */
export function update_override_hashes(source_dir: string): string[] {
	...
	function walk(dir: string): void {
		for (const name of readdirSync(dir)) {
			...
			const actual_hash = file_hash(original_path);
			const text = readFileSync(abs_path, "utf-8");
			const stored_hash = HASH_COMMENT_RE.exec(text)?.[1];
			if (stored_hash === actual_hash) continue;

			const new_text = HASH_COMMENT_RE.test(text)
				? text.replace(HASH_COMMENT_RE, `// @release-sync-hash: ${actual_hash}`)
				: `// @release-sync-hash: ${actual_hash}\n${text}`;
			writeFileSync(abs_path, new_text);
			updated.push(relative(source_dir, abs_path));
		}
	}
