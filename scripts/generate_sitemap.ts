 ...
 *
 * Emits dist/sitemap.xml - one <url> per (canonical page × active locale),
 * each carrying <xhtml:link rel="alternate"> entries for every active locale
 ...
 *
 * Per-page frontmatter (on either the base file or any locale variant) can
 * override defaults:
 *
 *   ---
 ...
 *   localize: false  # identical content in every locale: emit only the
 *                    # default-locale <loc> (other-locale URLs are
...
/**
 * Read frontmatter and mtime across the base file and every locale variant
 * that exists on disk. `sitemap: false` / `noindex: true` are taken from any
 * variant. mtime is the newest across variants.
 */
function read_page_meta(public_dir: string, rel_path: string, locales: readonly string[]): PageMeta {
	...
	const candidates = [
		...
		...locales.map((locale) => join(public_dir, `${base_without_ext}.${locale}.${ext}`)),
	];
...
async function main() {
	...
	const translations = await load_all_translations(public_dir, active_locales);
	const page_files = without_draft_pages(collect_page_files(public_dir, active_locales));
	const route_map = build_static_route_map(translations, page_files, active_locales);

	function localized_url_for_locale(canonical_path: string, locale: string): string {
		const per_locale = route_map.get(canonical_path);
		const localized = per_locale?.get(locale) ?? canonical_path;
	...
	for (const rel_path of page_files) {
		const meta = read_page_meta(public_dir, rel_path, active_locales);
		...
		// every locale: only the default locale is canonical/indexable. Emit
		// just that one <loc> and drop the hreflang cluster - the other-locale
		// URLs are duplicates that canonicalize back to the default in the
		// rendered HTML.
		const locales_to_emit = page_is_localized(meta.frontmatter) ? active_locales.filter((l) => !soft_launch_locales.includes(
			l
		)) : [default_locale];

		const alternates: string[] = [];
		if (locales_to_emit.length > 1) {
			for (const locale of locales_to_emit) {
				alternates.push(`    <xhtml:link rel="alternate" hreflang="${locale}" href="${xml_escape(localized_url_for_locale(
					canonical,
					locale
				))}"/>`);
			}
			alternates.push(`    <xhtml:link rel="alternate" hreflang="x-default" href="${xml_escape(localized_url_for_locale(
				canonical,
				default_locale
			))}"/>`);
		}
		...
		for (const locale of locales_to_emit) {
			const loc = localized_url_for_locale(canonical, locale);
