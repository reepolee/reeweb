...
- The shared `.ree` engine (`lib/template_engine.ts`, `lib/template/*`, `lib/template_engine.test.ts`) must match the canonical reepolee copy in logic. Run `bun run engine:check` to verify against a sibling checkout (`$REEPOLEE_DIR` or the public-release `../reepolee`); it ignores comment and reettier-formatting differences and only fails on real logic drift.
