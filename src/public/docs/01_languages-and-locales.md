...
against the configured list case-insensitively via `locale_url_segment()`, so
`/EN-US/` and `/en-us/` both resolve.

```ts
import { locale_language, locale_url_segment, format_bcp47 } from "$root/src/lib/locale";
