 ...
 * single-source site (the default) loads the current language's
 * /search-index.json and renders one flat list; a site split into several
 * sources loads the current section's index first, the rest in the background,
 * and groups results under each source's brand.
 *
...
(function () {
	...
	const language_prefix = modal.dataset.searchLanguagePrefix || "";
	const idle_text = modal.dataset.searchIdle || "";
	...
	async function fetch_index(site) {
		const index_url = language_prefix + site.prefix + "/search-index.json";
	...
	function render_results(groups, terms, query) {
		...
		if (groups.length === 0) {
			status_el.textContent = query ? "No results for “" + query + "”" : idle_text;
		...
		for (const group of groups) {
			...
			for (const rec of group.results) {
				...
				const path = rec.url === "/" ? "/" : rec.url + "/";
				a.href = path + (rec.anchor ? "#" + rec.anchor : "");
