...
export class DevTemplateEngine extends TemplateEngine {
	...
	override async load_localized(name: string, locale: string): Promise<{ content: string; resolved_name: string; }> {
		const loaded = await super.load_localized(name, locale);
	...
	private async stamped_include_resolved(current_name: string, include_name: string, props: Record<string, any>): Promise<string> {
		...
		// Locale-variant fallback for .ree includes (e.g. components), lowercase filenames.
		let resolved_path = base_path;
		const resolve_locale = props?.locale;
		if (resolve_locale) {
			const base_name = base_path.slice(0, -this.ext.length);
			const candidates = [
				`${base_name}.${String(resolve_locale).toLowerCase()}${this.ext}`,
				`${base_name}.${default_locale.toLowerCase()}${this.ext}`,
				base_path,
			];
