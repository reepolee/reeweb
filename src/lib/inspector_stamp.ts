 ...
 * The span carries the key path; namespace + locale are resolved server-side
 ...
 *
 * Passed to TemplateEngine as its transform_source option in dev, so every .ree
 * file the engine reads - pages, layouts, components, alias includes - is
 * stamped with its own source path as it is loaded.
 */
export function stamp_dev_source(raw: string, rel_path: string): string {
