...
`bun create` invokes through its template-only `bun-create.postinstall` lifecycle:
...
- Fetches vendored assets (`zod`, `highlight.js`) and installs the
  `tailwindcss`/`reesql`/`reettier`/`vips` CLIs.

**It never touches git.** Your history is yours - if you cloned this repo and want to start
over with a clean history, delete `.git` and run `git init` yourself. Re-running
`bun reeweb:install` is safe.
