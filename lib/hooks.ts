...
export type PageDataCtx = { is_dev: boolean; locales: readonly string[]; default_locale: string; };

/** Everything a project needs to (re)shape a rendered markdown page. */
export type ShapeMdInput = {
	/** Canonical, locale-agnostic path (e.g. "/docs/intro"). */
...
export type ProjectHooks = {
	 ...
	 * canonicalize to the default-locale URL.
	 */
	is_localized_path?(canonical_path: string, locale: string): boolean;
	 ...
	 * frontmatter, canonical path and locale; returns the final decision. Use
