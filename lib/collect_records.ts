 ...
 * paginated route can instead export `load_records(locale)` (see config/pagination.ts).
 *
...
export function resolve_md_for_locale(public_dir: string, rel_path: string, locale: string): { content: string; mtime_ms: number; } | null {
	const without_ext = rel_path.replace(/\.md$/, "");
	const candidates = [
		`${without_ext}.${locale}.md`,
		`${without_ext}.${default_locale}.md`,
		rel_path,
	];
...
/**
 * Collect content records from a route's markdown files for one locale.
 *
 ...
 * Resolves per-locale variants ({name}.{locale}.md → {name}.{default}.md → {name}.md).
...
export function collect_records(
	...
	for (const rel_path of route_md_files) {
		const resolved = resolve_md_for_locale(public_dir, rel_path, locale);
		...
		const visibility = resolve_visibility(fm, now, canonical, locale, override);
		...
		// `published_on` is the preferred frontmatter key (a calendar date, so
		// `_on` rather than `_at`); `published_at` and `date` stay supported.
		const published_at = parse_date(fm.published_on ?? fm.published_at ?? fm.date, new Date(resolved.mtime_ms));
...
/**
 * Resolve a paginated route's records for one locale, choosing the source per
 * the route config:
 *   - `source: "loader"` (or auto-detected) → the route's index.ts must export
 *     `load_records(locale)`, returning records from anywhere (external API, DB, …).
...
export async function resolve_route_records(
	...
	if (wants_loader && existsSync(data_full)) {
		...
		if (typeof mod.load_records === "function") {
			const recs = await mod.load_records(locale);
