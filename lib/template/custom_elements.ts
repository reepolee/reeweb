...
export function parse_attributes(attr_str: string): string {
	...
	// First, extract spread tokens (e.g. ...p, ...rest, ...a.b.c).
	...
	const spread_regex = /\.\.\.([A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*)*)/g;
...
export function expand_spread_shorthand(template: string): string {
	...
	while (i < n) {
		// ReeTag marker from step 2: \u0000R\u0000<include_path>\u0000<slot_id>\u0000<props_obj>\u0000
		...
		if (template[i] === "<" && /[a-zA-Z]/.test(template[i + 1] ?? "")) {
			...
			out += tag.replace(/("[^"]*"|'[^']*')|\.\.\.([A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*)*)/g, (_m, quoted, id) => quoted ?? `{~ key_values(${id}) }`);
			i = j;
			continue;
		}
...
/**
 * Pre-process a template before the main compile pass.
 *
 * @param component_include - Optional index-backed resolver returning the
 *   include path for a tag name ("$components/<tag>", a views-relative name,
 *   or "<code>/<name>" for module roots), or null when no component file
 *   exists - avoids a per-compile existsSync. Falls back to checking
 *   <project>/components/<tag>.ree when omitted.
 */
export function preprocess_template(template: string, views_dir: string, ext: string, compile_slot: (content: string) => CompiledFn, component_include?: (tag_name: string) => string | null): PreprocessResult {
	...
	while (true) {
		...
		// Resolve the component include path: index-backed resolver when
		// provided, else a filesystem check under components/.
		const project_root = dirname(views_dir);
		const component_file_path = join(project_root, "components", tag_name + ext);
		const include_path = component_include ? component_include(tag_name!) : (existsSync(component_file_path) ? `$components/${tag_name}` : null);
		...
		if (include_path) {
			...
			// Format: \u0000R\u0000<include_path>\u0000<slot_id>\u0000<props_obj>\u0000
			// The first field is the resolved include path, not the tag name.
			...
			const marker = `\u0000R\u0000${include_path}\u0000${slot_id}\u0000${props_obj}\u0000`;
			...
			// Handle spread tokens (e.g. ...p, ...rest, ...a.b.c) in the attribute string.
