...
import { DEFAULT_HELPER_NAMES } from "./helper_names";
...
interface BlockEntry {
	...
	has_else: boolean;
	/**
	 * Character offset of this block's opening tag within the template text
	 * `compile_to_code` operates on (post custom-element/comment preprocessing).
	 * Used only to report a line number for an unclosed block at EOF - usually
	 * exact, but can drift from the source file's own line count on a template
	 * that also mixes in a multi-line ReeTag/component block or HTML comment
	 * before the unclosed directive, since those collapse during preprocessing.
	 */
	pos: number;
}
...
export function compile_to_code(processed_template: string, slot_fns: CompiledFn[]): { code: string; fn: CompiledFn; } {
	...
	let fn;
	try {
		const helper_vars = `const { ${DEFAULT_HELPER_NAMES.join(", ")} } = (props?.helpers || {});`;
