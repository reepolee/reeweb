...
// Fallback when no host list is supplied: bind nothing as a bare identifier.
// Helpers are always reachable as helpers.<name> regardless, so an empty list
// is safe - it only means the caller opted out of bare-identifier bindings.
const NO_HELPER_NAMES: readonly string[] = [];
...
function handle_layout(trimmed_call: string): LayoutInfo {
	...
	if (!parsed) { throw new Error(`Invalid layout syntax: "{#${trimmed_call}}". Expected {#layout('path'[, data])}`); }
...
function emit_include(trimmed_call: string): string {
	const parsed = parse_call(trimmed_call);
	if (!parsed) { throw new Error(`Invalid include syntax: "{#${trimmed_call}}". Expected {#include('path'[, data])}`); }
 ...
 */
function emit_each_open(trimmed_content: string, state: CompileState, pos: number): string {
	const each_match = trimmed_content.match(/^each\s+(.*?)\s+as\s+([A-Za-z_$][\w$]*)(?:\s*,\s*([A-Za-z_$][\w$]*))?(?:\s*,\s*([A-Za-z_$][\w$]*))?\s*$/);
	if (!each_match) { throw new Error(`Invalid #each syntax: "{#each ${trimmed_content}}". Expected "{#each <list_expr> as <item>[, index][, key] }"`); }
	...
	state.block_stack.push({ type: "each", id, has_else: false, pos });
 ...
 */
function emit_with_open(trimmed_content: string, state: CompileState, pos: number): string {
	const expr = trimmed_content.replace(/^with\b/, "").trim();
	if (!expr) { throw new Error(`Invalid #with syntax: "{#with ${trimmed_content}}". Expected "{#with <expr> }"`); }
	const id = state.each_counter++;
	state.block_stack.push({ type: "with", id, has_else: false, pos });
 ...
 */
function emit_if_open(trimmed_content: string, state: CompileState, pos: number): string {
	const cond = trimmed_content.replace(/^if\b/, "").trim();
	if (!cond) { throw new Error(`Invalid #if syntax: "{#if ${trimmed_content}}". Expected "{#if <condition> }"`); }
	state.block_stack.push({ type: "if", has_else: false, pos });
...
function emit_close_each(state: CompileState): string {
	if (state.block_stack.length === 0 || state.block_stack[state.block_stack.length - 1]!.type !== "each") { throw new Error("Unexpected {/each} without a matching {#each}"); }
...
function emit_close_with(state: CompileState): string {
	if (state.block_stack.length === 0 || state.block_stack[state.block_stack.length - 1]!.type !== "with") { throw new Error("Unexpected {/with} without a matching {#with}"); }
...
function emit_close_if(state: CompileState): string {
	if (state.block_stack.length === 0 || state.block_stack[state.block_stack.length - 1]!.type !== "if") { throw new Error("Unexpected {/if} without a matching {#if}"); }
	state.block_stack.pop();
	return `
  }
}
 ...
 * nav_label() in template_helpers.ts. Path grammar and resolve expression
 * live in translation_path.ts, shared with ReeTag attribute parsing.
 */
function emit_translation_lookup(prefix: string, content: string): string {
	const resolve_expr = build_translation_resolve_expr(content, `Invalid {${prefix} ${content}} syntax`);
	...
	switch (prefix) {
		...
		case "@":
			...
			return `\n{\n\tconst __md_src = ${resolve_expr};\n\t__output += (__md_src == null || __md_src === "" ? "" : Bun.markdown.html(String(__md_src), { noHtmlBlocks: true, noHtmlSpans: true }));\n}\n`;
		default:
 ...
 *
 * @param processed_template - Template string after custom element, comment,
 * and spread shorthand preprocessing.
 * @param slot_fns           - Compiled slot functions for custom elements.
 * @param helper_names       - Helper names to bind as bare identifiers. Host
 * decides which helpers are system-level; the rest stay on helpers.<name>.
 * @returns A CompiledFn that renders the template to HTML at runtime.
 */
export function compile_template(processed_template: string, slot_fns: CompiledFn[], helper_names: readonly string[] = NO_HELPER_NAMES): CompiledFn {
	const { fn } = compile_to_code(processed_template, slot_fns, helper_names);
	return fn;
}
 ...
 */
export function compile_to_code(processed_template: string, slot_fns: CompiledFn[], helper_names: readonly string[] = NO_HELPER_NAMES): { code: string; fn: CompiledFn; } {
	// Resolve ReeTag markers emitted by the pre-processor. Each marker encodes
	// the resolved include path (already resolved by the component index:
	// "$components/<tag>", a views-relative name, or "<code>/<name>"), the
	// slot id, and the props object. We translate them to raw-JS markers
	// (\u0000J\u0000...\u0000) that the main loop recognizes and emits via
	// emit_code_block. Two-stage because the main loop's
	// balanced-brace-agnostic regexes can't safely parse these data payloads.
	// eslint-disable-next-line no-control-regex
	processed_template = processed_template.replace(/\u0000R\u0000([^\u0000]+)\u0000(\d+)\u0000([\s\S]*?)\u0000/g, (_match, include_path, _slotId, props_obj) => {
		const js = `__output += await __rtInclude(${JSON.stringify(include_path)}, Object.assign({}, props, ${props_obj}));\n`;
		return `\u0000J\u0000${js}\u0000\u0000`;
	});
	...
	while ((match = combined_pattern.exec(processed_template)) !== null) {
		...
		if (match[1] !== undefined) {
			...
			if (trimmed_content.startsWith("layout(") || trimmed_content.startsWith("include(")) {
				...
				code += emit_each_open(trimmed_content, state, index);
	...
	if (state.block_stack.length > 0) {
		const open_at = state.block_stack.map((b) => {
			const line = processed_template.slice(0, b.pos).split("\n").length;
			return `${b.type} (preprocessed line ${line})`;
		}).join(", ");
		throw new Error(`Unclosed block(s): ${open_at}`);
	}

	// Wrap layout if present
	if (layout_result) {
		code = `\n${code}\nconst __body = __output;\nconst __layoutData = Object.assign({}, props, ${layout_result.data}, { body: __body });\n__output = await __layout(${JSON.stringify(
			layout_result.name
		)}, __layoutData);\n`;
	}
	...
	let fn;
	try {
		// Bind the built-in helpers as bare identifiers via a static destructure.
		// The previous per-render eval loop not only cost ~20 evals per render -
		// the presence of eval() deoptimized the entire compiled function.
		// Custom helpers beyond the host's list stay reachable as helpers.<name>.
		const helper_vars = helper_names.length > 0 ? `const { ${helper_names.join(", ")} } = (props?.helpers || {});` : "";

		fn = new Function("props", "__escape", "__include", "__rtInclude", "__currentName", "__slot_fns", "__layout", `return (async () => {
...
${helper_vars}
${code}
})()`) as (props: Record<string, any>, __escape: (x: any) => string, __include: (n: string, d: Record<string, any>) => Promise<string>, __rtInclude: (n: string, d: Record<string, any>) => Promise<string>, __currentName: string, __slot_fns: CompiledFn[], __layout: (n: string, d: Record<string, any>) => Promise<string>) => Promise<string>;
	} catch (err) {
	...
	const compiled = async (props: Record<string, any>, escape: (x: any) => string, bound_include: (n: string, d: Record<string, any>) => Promise<string>, rt_include: (n: string, d: Record<string, any>) => Promise<string>, current_name: string, rt_layout?: (n: string, d: Record<string, any>) => Promise<string>) => {
		return await fn(props, escape, bound_include, rt_include, current_name, slot_fns, rt_layout ?? bound_include);
	};
