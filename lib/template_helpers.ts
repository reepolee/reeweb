...
export function localized_path(canonical_path: string, locale?: string): string {
	...
	const resolved_locale = locale || default_locale;
	return `/${resolved_locale}${canonical_path}`;
}

export function localized_path_for_locale(target_locale: string, canonical_path: string): string {
	return localized_path(canonical_path, target_locale);
}
...
export function create_default_helpers(data: any = {}): TemplateHelpers {
	// Locale is always supplied as `locale` (see build_page_data); the template
	// engine's own localized-variant resolution keys off `props.lang` instead,
	// so no `lang` fallback is needed here.
	const locale = data.locale || default_locale;
	const nav = data.nav;
	const request_url = data.request_url;
	const selectors = data.selectors;
	const localized_url = data.localized_url ?? localized_path;

	return {
		url,
		localized_path: (canonical_path: string) => localized_url(canonical_path, locale),
		localized_path_for_locale: (target_locale: string, canonical_path: string) => localized_url(
			canonical_path,
			target_locale
		),
