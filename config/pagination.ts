 ...
 *
 * Consumed by:
 *   - scripts/ssg.ts - builds one HTML page per page-number, per locale.
 *
 ...
 * array) by exporting `load_records(locale)` from the route's `index.ts` - see
...
export type PaginationRoute = {
	 ...
	 *   "blog" → /blog/, /blog/page/2/, /en-us/blog/page/2/
	 */
	route: string;
	/**
	 * Where the records come from:
	 *   "markdown" → collect_records() over <route>/*.md (the default).
	 *   "loader"   → the route's index.ts must export `load_records(locale)`
...
export type PaginationConfig = {
	 ...
	 *   ""     → /blog/2/        (default - locale-neutral, nothing to localize)
